...
router.route('/concerts/:id').get((req, res) => {
  const result = db.concerts.filter(record => record.id === parseInt(req.params.id));
  if (result.length) {
    res.json(...result);
  }
  else {
...
router.route('/concerts/:id').put((req, res) => {
  const { author, text } = req.body;
  const selectedRecord = db.concerts.find(record => record.id === parseInt(req.params.id));
...
router.route('/concerts/:id').delete((req, res) => {
  ...
  for (const record of db.concerts) {
